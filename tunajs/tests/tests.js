describe("In Tuna", function () {
	...
	describe("a Bitcrusher node", function () {
		...
		it("will have default values set", function () {
			expect(bitcrusher.bits)
				.toEqual(4);
			expect(bitcrusher.normfreq)
				.toBeCloseTo(0.1, 1);
			expect(bitcrusher.bufferSize)
				.toEqual(4096);
			expect(bitcrusher.bypass)
				.toBeFalsy();
		});
		...
		it("will be activated", function () {
			...
			expect(bitcrusher.activateCallback)
				.toHaveBeenCalled();
		});
	...
	describe("a Cabinet node", function () {
		...
		it("will will have default values set", function () {
			expect(cabinet.makeupGain.value)
				.toEqual(1);
			expect(cabinet.bypass)
				.toBeFalsy();
		});
		...
		it("will be activated", function () {
			...
			expect(cabinet.activateCallback)
				.toHaveBeenCalled();
		});
	...
	describe("a Chorus node", function () {
		...
		it("will have default values set", function () {
			expect(chorus.rate)
				.toBeCloseTo(1.5, 1);
			expect(chorus.feedback)
				.toBeCloseTo(0.4, 1);
			expect(chorus.delay)
				.toBeCloseTo(0.0002 * (Math.pow(10, 0.0045) * 2), 5);
			expect(chorus.depth)
				.toBeCloseTo(0.7, 1);
			expect(chorus.bypass)
				.toBeFalsy();
		});
		...
		it("will have passed values set", function () {
			...
			expect(chorus.feedback)
				.toBeCloseTo(0.2, 1);
			expect(chorus.delay)
				.toBeCloseTo(0.0002 * (Math.pow(10, 0.6) * 2), 5);
			expect(chorus.depth)
				.toBeCloseTo(0.5, 1);
			expect(chorus.bypass)
				.toBeTruthy();
		...
		it("will be activated", function () {
			...
			chorus.bypass = true;
			chorus.bypass = false;
			expect(chorus.activateCallback)
				.toHaveBeenCalled();
		});
	...
	describe("a Compressor node", function () {
		...
		it("will have default values set", function () {
			expect(compressor.threshold.value)
				.toEqual(-20);
			expect(compressor.release.value)
				.toEqual(250 / 1000);
			expect(compressor.makeupGain.value)
				.toBeCloseTo(1.12, 2);
			expect(compressor.attack.value)
				.toBeCloseTo(1 / 1000, 2);
			expect(compressor.ratio.value)
				.toEqual(4);
			expect(compressor.knee.value)
				.toEqual(5);
			expect(compressor.automakeup)
				.toBeFalsy();
			expect(compressor.bypass)
				.toBeFalsy();
		});
		...
		it("will have passed values set", function () {
			...
			expect(compressor.threshold.value)
				.toEqual(-35);
			expect(compressor.release.value)
				.toBeCloseTo(200 / 1000, 3);
			expect(compressor.makeupGain.value)
				.toBeCloseTo(1.33, 2);
			expect(compressor.attack.value)
				.toBeCloseTo(2 / 1000, 3);
			expect(compressor.ratio.value)
				.toEqual(3);
			expect(compressor.knee.value)
				.toEqual(4);
		...
		it("will be activated", function () {
			...
			expect(compressor.activateCallback)
				.toHaveBeenCalled();
		});
	...
	describe("a Delay node", function () {
		...
		it("will have default values set", function () {
			expect(delay.cutoff.value)
				.toBeCloseTo(20000, 1);
			expect(delay.delayTime.value)
				.toBeCloseTo(0.1, 1);
			expect(delay.feedback.value)
				.toBeCloseTo(0.45, 2);
			expect(delay.wetLevel.value)
				.toBeCloseTo(0.5, 1);
			expect(delay.dryLevel.value)
				.toBeCloseTo(1, 1);
			expect(delay.bypass)
				.toBeFalsy();
		});
	...
	describe("a Convolver node", function () {
		...
		it("will will have default values set", function () {
			expect(convolver.highCut.value)
				.toEqual(22050);
			expect(convolver.lowCut.value)
				.toEqual(20);
			expect(convolver.wetLevel.value)
				.toEqual(1);
			expect(convolver.dryLevel.value)
				.toEqual(1);
			expect(convolver.level.value)
				.toEqual(1);
			expect(convolver.bypass)
				.toBeFalsy();
		});
		...
		it("will be activated", function () {
			...
			expect(convolver.activateCallback)
				.toHaveBeenCalled();
		});
	...
	describe("a EnvelopeFollower node", function () {
		...
		it("will have default values set", function () {
			expect(envelope.attackTime)
				.toEqual(0.003);
			expect(envelope.releaseTime)
				.toEqual(0.5);
			expect(envelope.bypass)
				.toBeFalsy();
		});
		...
		it("will be activated", function () {
			...
			expect(envelope.activateCallback)
				.toHaveBeenCalled();
		});
	...
	describe("a Filter node", function () {
		...
		it("will have default values set", function () {
			expect(filter.frequency.value)
				.toEqual(800);
			expect(filter.Q.value)
				.toEqual(1);
			expect(filter.gain.value)
				.toEqual(0);
			expect(filter.filterType)
				.toEqual("lowpass");
			expect(filter.bypass)
				.toBeFalsy();
		});
		...
		it("will be activated", function () {
			...
			filter.bypass = true;
			filter.bypass = false;
			expect(filter.activateCallback)
				.toHaveBeenCalled();
		});
	...
	describe("an LFO node", function () {
		...
		it("will have default values set", function () {
			expect(lfo.frequency)
				.toEqual(1);
			expect(lfo.offset)
				.toEqual(0.85);
			expect(lfo.oscillation)
				.toEqual(0.3);
			expect(lfo.phase)
				.toEqual(0);
			expect(lfo.bypass)
	...
	describe("a MoogFilter node", function () {
		...
		it("will have default values set", function () {
			expect(filter.bufferSize)
				.toEqual(4096);
			expect(filter.processor.cutoff)
				.toEqual(0.065);
			expect(filter.processor.resonance)
				.toEqual(3.5);
			expect(filter.bypass)
				.toBeFalsy();
		});
	...
	describe("a Phaser node", function () {
		...
		it("will have default values set", function () {
			...
			expect(phaser.depth)
				.toEqual(0.6);
			expect(phaser.feedback)
				.toEqual(0.7);
			expect(phaser.stereoPhase)
				.toEqual(40);
			expect(phaser.baseModulationFrequency)
				.toEqual(700);
			expect(phaser.bypass)
				.toBeFalsy();
		});
		...
		it("will be activated", function () {
			...
			phaser.bypass = true;
			phaser.bypass = false;
			expect(phaser.activateCallback)
				.toHaveBeenCalled();
		});
	...
	describe("a PingPongDelay node", function () {
		...
		it("will have default values set", function () {
			expect(delay.delayTimeLeft)
				.toEqual(200);
			expect(delay.delayTimeRight)
				.toEqual(400);
			expect(delay.feedbackLevel.gain.value)
				.toBeCloseTo(0.3, 2);
			expect(delay.wet.gain.value)
				.toBeCloseTo(0.5, 2);
			expect(delay.bypass)
				.toBeFalsy();
		});
	...
	describe("a Tremolo node", function () {
		...
		it("will have default values set", function () {
			expect(tremolo.intensity)
				.toEqual(0.3);
			expect(tremolo.stereoPhase)
				.toEqual(0);
			expect(tremolo.rate)
				.toEqual(5);
			expect(tremolo.bypass)
				.toBeFalsy();
		});
		...
		it("will be activated", function () {
			...
			expect(tremolo.activateCallback)
				.toHaveBeenCalled();
		});
	...
	describe("a WahWah node", function () {
		...
		it("will have default values set", function () {
			expect(wahwah.automode)
				.toBeTruthy();
			expect(wahwah.baseFrequency)
				.toEqual(500);
			expect(wahwah.excursionOctaves)
				.toEqual(2);
			expect(wahwah.sweep)
				.toBeCloseTo(0.0062, 4);
			expect(wahwah.resonance)
				.toEqual(10);
			expect(wahwah.sensitivity)
				.toBeCloseTo(3.16, 2);
			expect(wahwah.bypass)
				.toBeFalsy();
		});
		...
		it("will have passed values set", function () {
			...
			expect(wahwah.automode)
				.toBeFalsy();
			expect(wahwah.baseFrequency)
				.toBeCloseTo(792.45, 2);
			expect(wahwah.excursionOctaves)
				.toEqual(3);
			expect(wahwah.sweep)
				.toBeCloseTo(0.0083, 4);
			expect(wahwah.resonance)
				.toEqual(11);
			expect(wahwah.sensitivity)
				.toBeCloseTo(3.98, 2);
		...
		it("will be activated", function () {
			...
			wahwah.bypass = true;
			wahwah.bypass = false;
			expect(wahwah.activateCallback)
				.toHaveBeenCalled();
		});
	});
	...
	describe("a Panner node", function () {
		...
		it("will have default values set", function () {
			expect(panner.pan.value)
				.toEqual(0);
			expect(panner.bypass)
				.toBeFalsy();
		});
		...
		it("will be activated", function () {
			...
			panner.bypass = true;
			panner.bypass = false;
			expect(panner.activateCallback)
				.toHaveBeenCalled();
		});
